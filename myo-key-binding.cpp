...
public:
	...
	void print()
	{
		...
		if (poseString.compare("waveIn") == 0 && geste){
			...
			// Test debut Activation du controle			
			timestamp = time(NULL);			
		}
		else if (poseString.compare("fingersSpread") == 0 && !geste && timestamp != -1 && (timeTmp -  timestamp > 0.5)){
		...
		else if (poseString.compare("rest") == 0 && !geste && timestamp != -1 && (timeTmp - timestamp > 0.5)){
			// Reset Activation du controle si fait par erreur
			geste = false;
			timestamp = -1;
		}		
		else if (poseString.compare("fingersSpread") == 0  && geste && timestamp == -1){
			// Test Debut Desactivation du controle
			timestamp = time(NULL);
		}
		else if (poseString.compare("fingersSpread") == 0 && geste && timestamp != -1 && (timeTmp - timestamp > 0.5)){
			...
			// Reset Desactivation du controle si fait par erreur
			timestamp = -1;
		}
